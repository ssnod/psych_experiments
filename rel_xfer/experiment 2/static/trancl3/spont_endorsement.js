var endorsement = {
    ...
    start: function(params, next) {
        ...
        endorsement.initElements(params)
        endorsement.initDebugWindow(params)
        ...
        trial = 0
        block = 0
        block_order = endorsement.generateBlockOrder(params['objects'], params['labels']) // <-- initialize the first block order (you'll do this every block)

        correctCategory = block_order[trial][1]
        endorseLabel = block_order[trial][2]
        instructionsTxt.text = "Is this a ".concat(endorseLabel,"?")
        endorsement.initStim(block_order[trial][0])
        ...
        continueBtn.bind('click tap', function () {
            ...
            if (trial == block_order.length) {
                ...
                if (block == params['n_blocks']) {
                    ...
                    block_order = endorsement.generateBlockOrder(params['objects'], params['labels'])
                }
            }
    ...
    generateBlockOrder: function (objects, labels) {
        let block = []
        idx = endorsement.shuffle([...Array(labels.length).keys()]) // from ben @ https://stackoverflow.com/a/10050831
        ...
        for (let i of idx) {
            ...
            if(labels[i] == "Doppa"){
                ...
                elabels = endorsement.shuffle(elabels)
