var obCo = {
    ...
    start: function(params, next) {
        ...
        obCo.initElements(params)
        obCo.initDebugWindow(params)
        ...
        trial = 0
        block = 0
        block_order = obCo.generateBlockOrder(params['objects'], params['labels']) // <-- initialize the first block order (you'll do this every block)

        correctCategory = block_order[trial][1]
        obCo.initLStim(block_order[trial][0])
        obCo.initRStim(block_order[trial][2])

        setTimeout( function(){
            instructionsTxt.text = "Both of these are a ".concat(correctCategory,".")
        ...
        responseBtns[category].btn.bind('click tap', function () {
            ...
            if (trial == block_order.length) {
                ...
                if (block == params['n_blocks']) {
                    ...
                    block_order = obCo.generateBlockOrder(params['objects'], params['labels'])
                }
            }
    ...
    createRandomOrder: function (objects, labels){
        ...
        idx = obCo.shuffle([...Array(labels.length).keys()]) // from ben @ https://stackoverflow.com/a/10050831
    ...
    generateBlockOrder: function (objects, labels) {
       ...
       rndmOrder = obCo.createRandomOrder(objects, labels)

       pairList = obCo.createPairs(rndmOrder)
       

    // create training block
        let block = []

        idx = obCo.shuffle([...Array(pairList.length).keys()]) // from ben @ https://stackoverflow.com/a/10050831
