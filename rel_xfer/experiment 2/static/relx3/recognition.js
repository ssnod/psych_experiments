var recognition = {
    ...
    start: function(params, next) {
        ...
        recognition.initElements(params)
        recognition.initDebugWindow(params)
        ...
        trial = 0
        block = 0
        block_order = recognition.generateBlockOrder(params['objects'], params['labels']) // <-- initialize the first block order (you'll do this every block)

        correctCategory = block_order[trial][1]
        recognition.initStim(block_order[trial][0])
            ...
            responseBtns[category].btn.bind('click tap', function () {
                ...
                if (trial == block_order.length) {
                    ...
                    if (block == params['n_blocks']) {
                        ...
                        block_order = recognition.generateBlockOrder(params['objects'], params['labels'])
                    }
                }
                setTimeout( function () {
                    setTimeout( function () {
    ...
    initElements: function (params) {
        ...
        instructionsTxt = space.display.text({
            ...
            align: "center",
            font: "bold 25px sans-serif",
            text: "Do you remember seeing this item during the memory phase?",
            fill: "rgba(0,0,0,1)",
            zIndex: "front",
        })
    ...
    generateBlockOrder: function (objects, labels) {
        let block = []
        idx = recognition.shuffle([...Array(labels.length).keys()]) // from ben @ https://stackoverflow.com/a/10050831
